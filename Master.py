...
for folder in folders:
    ...
    for file in files:
        ...
        for color in image_colors:
            ...
            for i in range(1,L_int+1):
                if tuple([color[1]]) in dict_of_HCV_Classes[i]:
                    row_counts[i-1] += color[0]
                    break
            else:
                i = np.int(classifier.predict(X_train, y_train)[0])
                row_counts[i-1] += color[0]
                dict_of_HCV_Classes[i].append(tuple([color[1]]))
        
        # Write the row info to the csv_file
        csv_rows.append(csv_row+row_counts)
